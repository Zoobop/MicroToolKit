...
namespace Micro
{
	...
	struct HashNode final
	{
		...
		constexpr HashNode(const HashNode& other) noexcept
			: Value(other.Value), Next(other.Next), Status(other.Status)
		{
		}

		constexpr HashNode(HashNode&& other) noexcept
			: Value(std::move(other.Value)), Next(other.Next), Status(other.Status)
		{
			other.Invalidate();
		}

		explicit constexpr HashNode(const TValue& value) noexcept
			: Value(value), Status(MemStatus::Valid)
		{
		}

		explicit constexpr HashNode(TValue&& value) noexcept
			: Value(std::move(value)), Status(MemStatus::Valid)
		{
		}

		constexpr ~HashNode() noexcept { Invalidate(); }
		...
		NODISCARD constexpr TValue& GetValue() const noexcept { return m_KeyValuePair.Component2; }
		NODISCARD constexpr bool IsValid() const noexcept { return Status == MemStatus::Valid; }

		// Operator Overloads
		HashNode& operator=(const HashNode& other) noexcept
		{
			if (this == &other)
				return *this;
	...
	private:
		Tuple<TKey, TValue> m_KeyValuePair;
	};
	...
	private:
		hashptr_t _NullNode = static_cast<_HashNode*>(0xcdcdcdcdcdcdcdcd);
